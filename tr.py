...
def movement_values(item_list):
    ...
    bk = Button(Canvas1, text="x", activeforeground="white", activebackground="#3385ff",
                    fg='black', bg='#3385ff', borderwidth=0, font=('Arial 16 bold'),).place(x=1280, y=0,height=18)
...
def group_analysis():
    ...
    E1 = Entry(f5,width=30,borderwidth="1",bg="#f7d065") #entry for group search
    E1.pack(padx=10,pady=20)
    ...
    conn = mysql.connector.connect(host ="localhost",
                                     ...
                                     db ="db")
    c= conn.cursor()
    c.execute('SELECT  bnk_group_name  FROM   app_bnk_group')
    dt=c.fetchall()
    ...
    def db_search(e):
        ...
        conn = mysql.connector.connect(host ="localhost",
                                            ...
                                            db ="db")
        c= conn.cursor()
        c.execute('SELECT bnk_group_name  FROM app_bnk_group WHERE bnk_group_name LIKE "%{}%"'.format(search))
        dt=c.fetchall()
        if dt:
            for items in dt:
                lb.insert(END,items)
